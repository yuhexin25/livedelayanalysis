export default function WelcomePage({ sourceMode, providerMode, onOpenDashboard, onViewMethodology, onExploreNetwork }) {
  ...
  return (
    <section className="welcome-landing">
      ...
      <h2>{isFlightAwareActive ? 'Live FAA + OpenSky + FlightAware Operational Metrics Platform' : 'Live FAA Advisory + OpenSky Traffic Signals + Estimated Operational Metrics Platform'}</h2>
      <p>
        This platform combines {isLive ? 'live FAA airport advisory/status feeds' : 'sample operational delay scenarios'},
        {isLive ? ' observed OpenSky aircraft traffic signals,' : ''}{isFlightAwareActive ? ' provider-backed operational metrics' : ' estimated operational metrics'},
        static route network data, and estimated network impact scoring to explore delay propagation, hub
        vulnerability, and airport network resilience.
      </p>
      <div className="provenance-row">
        ...
        <span><i className="provenance-dot traffic-dot" />OpenSky traffic proxy</span>
      ...
      <p className="honesty-note">
        FAA advisory/status data is observed. OpenSky is observed aircraft position/activity data, not official delay
        data. Delay and cancellation metrics are estimated until FlightAware or another live flight-level provider is configured.
      </p>
    </section>
  );
}
